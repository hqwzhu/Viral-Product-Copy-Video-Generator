...
```powershell
python scripts\promotion_manager.py `
...
一次成功或部分成功的运行会在 `--out-dir` 写入产品事实、来源和状态、竞品/证据任务、平台内容草稿、口播稿与分镜、媒体目录、发布队列、发布包、审计报告和证据收件箱入口。具体文件名以当前版本报告为准，文档不会把可选媒体当成必有成品。
