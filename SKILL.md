...
- `reports/promotion-manager/final-run/final-capability-run.{json,md}` when `scripts/final_capability_runner.py` runs the highest-automation safe flow: Codex-first product reading, promotion cycles, multi-query viral discovery, publish readiness, browser-assisted publish payloads, real evidence recovery, next-round optimization, and audits.
