...
For the detailed gap-to-100% plan, operator steps, open-source references, and acceptance evidence for each module, see [100% Completion Roadmap](100-percent-completion-roadmap.md). For beginner-friendly Chinese steps, see [100% 完成指南](zh-CN/100-percent-completion-guide.md) or generate `operator-action-checklist.zh-CN.md`.
...
| Optional Firecrawl-style web data backend for Search, Scrape, Map, Crawl, Batch Scrape | `web_data_provider.py`, plus integrations in `product_url_reader.py`, `product_url_discovery.py`, and `platform_search_browser.py` | Optional `FIRECRAWL_API_KEY` or isolated self-hosted Firecrawl. Without it, existing browser/static/user-evidence fallback remains active. |
| Firecrawl-style Interact without unsafe automation | `web_data_provider.py interact-plan` creates a plan-only action list, blocked-action list, and stop conditions for public pages | Interact must not bypass login, captcha, risk controls, account verification, final publish, like, follow, comment, or DM actions. Use official APIs or manual/browser-assisted workflows for platform side effects. |
| AiToEarn-style platform capability registry and monetization blueprint | `platform_capabilities.py` and `final_capability_audit.py` capability rows | Live creator marketplace, payout provider, legal/tax/account onboarding, and automatic settlement are still future work. Cookie/simulated-login automation is intentionally rejected. |
| AiToEarn Relay-style bridge | `platform_capabilities.py` documents `relayBridgePolicy` as a temporary optional bridge | It can be used only with explicit operator acceptance of the third-party trust boundary. It must later migrate to official APIs or auditable self-hosted workers, and cannot be the core open-source safety promise. |
...
| Generate publish packages first; keep auto-publish ports reserved | Current policy: manual publish packages are the primary path; auto-publish ports are reserved for later official API-only upgrades. `publish_queue.py`, `publish_readiness_runner.py`, `launch_unlock_pack.py`, `browser_publish_assistant.py`, `browser_publish_form_fill.py`, and `browser_publish_session.py` are the default path. `publish_executor.py`, `youtube_oauth_publish.py`, and `youtube_credential_check.py` remain dry-run-first official API ports for GitHub and YouTube; Douyin is browser-assisted/manual by default and its official executor is reserved for a future verified authorization path. YouTube uses `google-api-python-client`; install with `python -m pip install -r requirements-youtube.txt`. | Manual final publish or visible browser review is still required by default. Future auto-publish execution requires official credentials, app review, account authorization, explicit `I_APPROVE_PUBLISH`, and `PUBLISH_DRY_RUN=false`. YouTube OAuth accepts `GOOGLE_OAUTH_*` or `YOUTUBE_CLIENT_*` client credential names. |
...
python scripts\web_data_provider.py --provider local --out-dir ".\promotion-output" search --query "AI product promotion"
