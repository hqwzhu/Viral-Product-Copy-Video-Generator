...
- Run `scripts/product_batch_runner.py` to read multiple product URLs first, execute one guarded local promotion cycle per ready product using structured snapshots when available, and optionally run product-driven multi-query viral discovery for every ready product.
