...
- `reports/promotion-manager/competitors/captured-search-results-<platform>.{json,md}` when `scripts/platform_search_capture.py` imports rendered search snapshots, saved HTML, copied text, or public static URLs; records include `contentDeconstruction` for hook/beat/video-architecture analysis when enough visible text exists
